...
const SelectTrigger = React.forwardRef(({ className, children, ...props }, ref) => {
  return (
    <div className={cn("relative", className)} ref={ref}>
      {children}
      <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 opacity-50 pointer-events-none z-10" />
    </div>
  )
})
...
const SelectValue = ({ placeholder, ...props }) => {
  return <option value="" disabled {...props}>{placeholder}</option>
}
