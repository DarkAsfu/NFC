...
import { AlertCircle, CheckCircle, Plus, X, Trash2, Edit2, Save, Image as ImageIcon } from 'lucide-react'
...
export default function EditProfilePage() {
  ...
  const handleAddItem = async (endpoint, data, setter) => {
    if (!user?.username) return
    try {
      const res = await api.post(endpoint.replace('{username}', user.username), { ...data, profile: profile?.id })
      setMessage({ type: 'success', text: 'Item added successfully' })
      fetchProfileData()
    } catch (err) {
      setMessage({ type: 'error', text: err?.response?.data?.detail || 'Failed to add item' })
    }
  }
          ...
          <TabsContent value='contact' className='space-y-4'>
            <Card>
              <CardHeader>
                ...
                <CardDescription>Add your website, address, and date of birth</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value='gallery' className='space-y-4'>
            <Card>
              <CardHeader>
                ...
                <CardDescription>Upload images to your gallery</CardDescription>
              </CardHeader>
              <CardContent>
                <p className='text-sm text-muted-foreground'>Gallery management coming soon...</p>
              </CardContent>
            </Card>
          </TabsContent>
...
function ContactInfoForm({ contactInfo, onSave, saving }) {
  const [formData, setFormData] = useState({
    ...
    dob: contactInfo?.dob ? new Date(contactInfo.dob).toISOString().split('T')[0] : ''
  })
