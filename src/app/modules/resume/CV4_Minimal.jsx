...
export default function CV4_Minimal({ user, profile, about, contactInfo, skills, experiences, educations, languages, certificates }) {
  ...
  return (
    <div className="bg-white text-gray-900 print:p-0" style={{ maxWidth: '210mm', margin: '0 auto', padding: '50px' }}>
      {/* Minimal Header */}
      <div className="text-center mb-12 pb-8 border-b border-gray-300">
        <h1 className="text-5xl font-light mb-3 text-gray-900 tracking-tight">{fullName}</h1>
        {profile?.bio && (
          <p className="text-gray-600 text-base font-light">{profile.bio}</p>
        )}
      ...
      <div className="space-y-10">
        ...
        {experiences?.length > 0 && (
          <section>
            <h2 className="text-sm font-normal text-gray-500 uppercase tracking-wider mb-6">Experience</h2>
            <div className="space-y-6">
              {experiences.map((exp) => (
                <div key={exp.id} className="flex justify-between items-start">
                  <div className="flex-1">
                    <h3 className="text-lg font-normal text-gray-900 mb-1">{exp.title || exp.position}</h3>
                    <p className="text-sm text-gray-600 mb-2">{exp.company}</p>
                    {exp.description && (
                      <p className="text-sm text-gray-700 font-light leading-relaxed">{exp.description}</p>
                    )}
                  </div>
        ...
        {educations?.length > 0 && (
          <section>
            <h2 className="text-sm font-normal text-gray-500 uppercase tracking-wider mb-6">Education</h2>
            <div className="space-y-4">
              {educations.map((edu) => (
                <div key={edu.id} className="flex justify-between items-start">
                  <div>
                    <h3 className="text-lg font-normal text-gray-900 mb-1">{edu.degree || edu.field_of_study}</h3>
        ...
        {skills?.length > 0 && (
          <section>
            <h2 className="text-sm font-normal text-gray-500 uppercase tracking-wider mb-4">Skills</h2>
        ...
        {languages?.length > 0 && (
          <section>
            <h2 className="text-sm font-normal text-gray-500 uppercase tracking-wider mb-4">Languages</h2>
        ...
        {certificates?.length > 0 && (
          <section>
            <h2 className="text-sm font-normal text-gray-500 uppercase tracking-wider mb-4">Certifications</h2>
            <div className="space-y-2">
              {certificates.map((cert) => (
                <div key={cert.id}>
                  <p className="text-sm text-gray-700">{cert.name || cert.title}</p>
